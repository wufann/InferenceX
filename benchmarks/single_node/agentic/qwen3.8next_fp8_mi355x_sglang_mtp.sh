...
# AgentX trace replay for Qwen3.8-Flash-Next FP8 on MI355X (gfx950) using
...
#
# MI355X is gfx950, so this arm is FP8 (Qwen/Qwen3.8-Flash-Next-FP8): the NVFP4
# checkpoint the Blackwell arms use needs SM100 tensor cores. Per PR #36601's
# release note FP8 runs single-GPU TP1 on gfx950 -- 172.8 GiB of FP8 weights fit
# on one MI355X (288 GB HBM), leaving room for the conc 8..16 KV pools, and
# every rank-crossing collective disappears. (TP>1 would also trip the FP8
# block-quant divisibility check: moe_intermediate_size 640 / TP is not a
# multiple of block_n 128 for TP2/4/8 -- use EP for that topology instead.)
#
...
# auto, EAGLE 3-step MTP), extended with the agentic framework's env contract,
# trace replay, metrics, and golden-AL simulation. The NVIDIA arms' flashinfer
# linear-attn backends and --mamba-ssm-dtype gymnastics do NOT apply on
# ROCm/aiter, so they are intentionally omitted; the aiter path picks its own
...
# at a populated local checkpoint dir (the usual MI355X case), skip the network.
...
# qwen3.8-flash-next-fp8 thinking_on curve at num_speculative_tokens=3
# (golden_al_distribution/qwen3.8next_mtp.yaml). EVAL_ONLY leaves this off and
# keeps real target verification so accuracy stays meaningful.
...
SGLANG_CMD=(
    ...
    --trust-remote-code
    "${PARALLEL_ARGS[@]}"
    # Verified single-node serving flags for Qwen3.8-Flash-Next-FP8 on MI355X.
